...
int main() {
	...
	/*Quadtree halfTree;
	Quadtree half2Tree;
	halfTree.buildTree(imgIn, 256);
	half2Tree = halfTree;
	imgOut = half2Tree.decompress();
	imgOut.writeToFile("outHalf.png");
	halfTree.clockwiseRotate();
	imgOut = halfTree.decompress();
	imgOut.writeToFile("rotationss.png");
	*/
	Quadtree PruneTree;
	PruneTree.buildTree(imgIn, 256);
	PruneTree.prune(1000);
	imgOut = PruneTree.decompress();
